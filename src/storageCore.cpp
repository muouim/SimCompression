...
bool StorageCore::saveChunks(char *data) {
    ...
    for (int i = 0; i < chunkNumber; i++) {
        ...
        if (fp2ChunkDB.query(originHash, tmpdata)) {
            ...
            if (!saveChunk((char *)originHash.c_str(), data + readSize, currentChunkSize)) {
                return false;
            }
        }
...
bool StorageCore::restoreRecipeAndChunk(char *fileNameHash, uint32_t startID, uint32_t endID,
    ...
    if (fileName2metaDB.query(DBKey, recipeName)) {
        ...
        for (int i = 0; i < (endID - startID); i++) {
            ...
            if (restoreChunk((char *)chunkHash.c_str(), chunkData)) {
                if (chunkData.length() != newRecipeEntry.chunkSize) {
                    cout<<chunkData.length() <<" "<<newRecipeEntry.chunkSize<<endl;
                    ...
                    //此处进行判断，该chunk是否做过delta，如果做过解压
                    memset(newChunk.logicData,0,MAX_CHUNK_SIZE);

                    memcpy(newChunk.logicData, (char *)chunkData.c_str(), newChunk.logicDataSize);
...
bool StorageCore::deleteRecipeAndChunk(char *fileNameHash, uint32_t startID, uint32_t endID) {
    ifstream RecipeIn;
    string readRecipeName;
    string recipeName;
    string DBKey(fileNameHash, FILE_NAME_HASH_SIZE);
    if (fileName2metaDB.query(DBKey, recipeName)) {
        ...
        for (int i = 0; i < (endID - startID); i++) {
            ...
            // PRINT_BYTE_ARRAY_STORAGE_CORE(stdout, (char*)newRecipeEntry.chunkHash, CHUNK_HASH_SIZE);
            if (deleteChunk((char *)chunkHash.c_str())) {
            

            } else {
                cerr << "StorageCore : can not delete chunk" << endl;
                return false;
            }
        }
        fileName2metaDB.delete_(DBKey);
        remove((char *)readRecipeName.c_str());
        return true;
    } else {
        return false;
    }
}
bool StorageCore::saveChunk(char * chunkhash, char *chunkData, int chunkSize) {
    ...
    keyForChunkHashDB_t key,tempkey;
    string ans;
    
    bool status = fp2ChunkDB.query(chunkHash, ans);
    if (status) {//判断新插入还是要更新
        memcpy(&tempkey, &ans[0], sizeof(keyForChunkHashDB_t));
        key.length = tempkey.length;
        key.deltaCount=tempkey.deltaCount;
        key.count=tempkey.count;
    }
    else {
    ...
    for(int i=0; i<key.length-2;i++) {
        ...
        if(chunkData[i]=='\0')chunkData[i]='\200';
    }
    //cout<<"save "<<key.length<<" "<<strlen(chunkData)<<endl;

    status = writeContainer(key, chunkData);
    if (!status) {
...
bool StorageCore::restoreChunk(char * chunkhash, std::string &chunkDataStr) {
    ...
    if (status) {
        ...
        char chunkData[key.length]={0};

        if (readContainer(key, chunkData)) {

            if(key.deltaCount>0) {
                string restoredata(chunkData);
                string basechunk,basehash(chunkData,CHUNK_HASH_SIZE);
                //cout<<"delta"<< endl;cout<<basehash<<" "<<chunkhash<< endl;                
                restoreChunk((char *)basehash.c_str(),basechunk);
                char nodelta[MAX_CHUNK_SIZE]={0};
                delta_apply(basechunk.c_str(),basechunk.size(),(char *)
                restoredata.substr(CHUNK_HASH_SIZE,restoredata.size()).c_str(), 
                restoredata.size()-CHUNK_HASH_SIZE,nodelta);
                
                chunkDataStr.resize(strlen(nodelta));
                chunkDataStr=nodelta;
            }
            else {
                //cout<<"nodelta "<<key.length<<" "<<strlen(chunkData)<<endl;
...
bool StorageCore::retriveChunk(char * chunkhash, std::string &chunkDataStr) {
...
bool StorageCore::deleteChunk(char * chunkhash) {
    ...
    if (status) {
        ...
        fp2ChunkDB.insert(chunkHash, dbValue);
...
bool Container::saveTOFile(string fileName) {
    ...
    containerDB.insert(fileName,to_string(this->used_));
